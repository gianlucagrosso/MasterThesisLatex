...
Patched Quantics Tensor Cross Interpolation (pQTCI) is an adaptive partitioning method \cite{Deng2008} based on rank-revealing cross approximation for high-dimensional tensors. Cross interpolation represents only a subroutine for our algorithm, hence, we will assume that the reader is familiar with all TCI's functionalities (cf. \prettyref{chap:QTCI} and Ref. \cite{Fernandez2024} otherwise). Since it is built upon the TCI framework, pQTCI also falls within the class of \emph{rank‑revealing} algorithms introduced in \prettyref{def:rkralg}.
...
\end{align}
with identical phyisical dimensions, $d^A_\ell = d^B_\ell$ $\forall \ell$, but possibly different virtual dimensions $\chi^A_\ell$, $\chi^B_\ell$. Their element-wise sum $\widetilde{C}_{\bsigma} =  \widetilde{A}_{\bsigma} + \widetilde{B}_{\bsigma}$ is still a tensor trained shaped as
\begin{equation}
...
\begin{align}
	\ket{\Psi} = \Psi_{\bsigma}\ket{\bsigma} &= \ket{\sigma_1}\ket{\sigma_2}\dots \ket{\sigma_\mL} \Psi_{\sigma_1,\sigma_2,\dots,\sigma_\mL} \\[6pt]
...
\begin{align}
	\label{eq:quantumMPS}
	\ket{\Psi} = \widetilde{\Psi}_{\bsigma}\ket{\bsigma} &=  \ket{\sigma_1}[M_1^{\sigma_1}]_{1 m_1}   \ket{\sigma_2}[M_2^{\sigma_2}]_{m_1 m_2} \cdots \ket{\sigma_\mL}[M_\mL^{\sigma_\mL}]_{m_{\mL -1} 1} \\[6pt]
...
\begin{align}
	...
	\nonumber \ket{\Psi^{p_\ell}} &= \ket{\sigma_1}\cdots\ket{\sigma_{\ell -1}}\ket{p_\ell}\ket{\sigma_{\ell +1}}\cdots\ket{\sigma_\mL} \Psi_{\sigma_1,\dots,\sigma_{\ell -1}, p_\ell, ,\sigma_{\ell +1}, \dots, \sigma_\mL}\\[6pt]
	&=  \ket{\sigma_1}\cdots\ket{\sigma_{\ell -1}}\ket{p_\ell}\ket{\sigma_{\ell +1}},\cdots\ket{\sigma_\mL}\Psi^{p_\ell}_{\sigma_1,\dots,\sigma_{\ell -1},\sigma_{\ell +1}, \dots, \sigma_\mL}\\[6pt]
...
This procedure resembles a quantum projection, \textit{i.e.} $\ket{\Psi^{p_\ell}} = \ket{p_\ell}\braket{p_\ell|\Psi}$. 
...
\raisebox{-4.5mm}{\includegraphics[scale=0.95]{figures/ProjectionTensor_lLabel.pdf}} = \begin{cases}
	...
	\end{cases}
\end{equation}
with bonds $m_{\ell-1}$ and $m_{\ell}$ of same dimension and where $0_{m,n}$ denotes the element $(m,n)$ of the zero matrix of dimension $\chi_{\ell-1}\times\chi_\ell$.
...
\end{equation}
where the first approximation reflects the error associated with an arbitrary MPS approximation and the sum is performed according to the element-wise TT addition described by \prettyref{eq:TensorTrainsSum} in the section above. Without loss of generality, the same line of reasoning can be applied to smaller subspaces
\begin{equation}
	...
	\label{eq:NslicesSum}
\end{equation}
taking $(\ell_1, \dots, \ell_N) \in \{1,\dots, \mL\}^{N(\leq \mL)}$ ($\ell_i \neq \ell_j$ for every $i\neq j$) without any specific order constraint. Whenever the sum \prettyref{eq:NslicesSum} retains a low rank, after recompression, $\widetilde{\Psi}^{+}_{\bsigma}$ represents good approximation of the original tensor $\Psi_{\bsigma}$.
...
\end{enumerate}
\endgroup
\footnotetext{A different choice for the starting index is possible, see below. For pedagogical purposes we start from $\sigma_1$.}
...
\begin{itemize}
	\item any subtensor that is further subdivided has its non-convergent TT approximation discarded, all pairs of index prefixes satisfy
	\begin{equation}
...
\prettyref{fig:patchSubdivision} illustrates this idea for a bivariate function.
...
\begin{figure}[htbp]
	...
	\caption{Evolution of the adaptive partitioning of the bivariate function $f(\boldsymbol{r})$ domain (cf. \prettyref{eq:localFunc} and \prettyref{eq:localFuncNewParams}), due to pQTCI. Both interleaved and fused ordering for the tensor $\mF_{\bsigma} = f(\boldsymbol{r}(\bsigma))$ are illustrated. Smaller maximum bond dimensions per patch $\chip$ render finer partitionings of the domain.}
...
We can then understand that there exists a dependence of $\Np$ by $\chip$. From the memory estimate in \prettyref{eq:patchMemoryEstimate}, and assuming the overall parameter budget of the final approximation to stay roughly constant, we anticipate the following scaling law
\begin{equation}
	\Np \propto 1 / \chip^2. 
...
\begin{figure}[htbp]
	...
	\caption{Performance of patched QTCI on the two–dimensional oscillatory function of \prettyref{eq:2Dfunction}.  $(a)$ Example of the domain subdivision produced with $\mR = 15$, $\chip = 10$, and $\tau = 10^{-4}$.  The partition is highly redundant and fails to align with the true feature layout of the function. $(b)$ Total number of TT parameters returned by pQTCI (solid curves) at three target tolerances, plotted against the bond-dimension cap $\chip$.  For comparison, the parameter count of a full-domain TCI compression is shown as a dotted line.  Once $\chi_{\text{patch}}\gtrsim 80$, every run collapses to a single patch and matches the TCI cost; for smaller caps, over-patching inflates the parameter count sharply.}
